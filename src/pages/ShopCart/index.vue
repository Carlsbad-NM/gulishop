...
export default {
  ...
  methods: {
    ...
    async deleteChecked() {
      try {
        await this.$store.dispatch("deleteCheckedShopCard");
        this.getShopCartList();
      } catch (error) {
        alert(error.message);
      }
  ...
  computed: {
    ...
    isAllChecked: {
      ...
      async set(val) {
        try {
          await this.$store.dispatch("updateAllIsChecked", val ? 1 : 0);
          this.getShopCartList();
        } catch (error) {
          alert(error.message);
        }
